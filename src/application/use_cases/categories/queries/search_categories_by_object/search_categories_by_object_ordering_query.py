from application.commons.queries import OrderingCriterionQuery
from application.commons.validators import CustomValidator
from application.error_catalog import ErrorConstants
from application.exceptions.errors import CustomValidationErrorVm
from .search_categories_by_object_ordering_dto import SearchCategoriesByObjectOrderingDto


class SearchCategoriesByObjectOrderingQuery(CustomValidator):

    def __init__(self, dto:SearchCategoriesByObjectOrderingDto):
        self.name = OrderingCriterionQuery(dto.name) if dto.name is not None else None
        self.description = OrderingCriterionQuery(dto.description) if dto.description is not None else None
        self.created_at = OrderingCriterionQuery(dto.created_at) if dto.created_at is not None else None
        
    def format_validation(self):
        errors = []
        
        if self.name is not None and not self.name.is_valid_ordering_operation():
            errors.append(CustomValidationErrorVm(
                error_code=ErrorConstants.SEARCH_CATEGORIES_BY_OBJECT_ORDERING_FORMAT00001.error_code,
                property_name=ErrorConstants.SEARCH_CATEGORIES_BY_OBJECT_ORDERING_FORMAT00001.property_name,                    
                error_message=ErrorConstants.SEARCH_CATEGORIES_BY_OBJECT_ORDERING_FORMAT00001.error_message.format(
                    order_by=self.name.operator
                )
            ))

        if self.description is not None and not self.description.is_valid_ordering_operation():
            errors.append(CustomValidationErrorVm(
                error_code=ErrorConstants.SEARCH_CATEGORIES_BY_OBJECT_ORDERING_FORMAT00002.error_code,
                property_name=ErrorConstants.SEARCH_CATEGORIES_BY_OBJECT_ORDERING_FORMAT00002.property_name,                    
                error_message=ErrorConstants.SEARCH_CATEGORIES_BY_OBJECT_ORDERING_FORMAT00002.error_message.format(
                    order_by=self.description.operator
                )
            ))
                            
        if self.created_at is not None and not self.created_at.is_valid_ordering_operation():
            errors.append(CustomValidationErrorVm(
                error_code=ErrorConstants.SEARCH_CATEGORIES_BY_OBJECT_ORDERING_FORMAT00003.error_code,
                property_name=ErrorConstants.SEARCH_CATEGORIES_BY_OBJECT_ORDERING_FORMAT00003.property_name,                    
                error_message=ErrorConstants.SEARCH_CATEGORIES_BY_OBJECT_ORDERING_FORMAT00003.error_message.format(
                    order_by=self.created_at.operator
                )
            ))
            
        return errors
