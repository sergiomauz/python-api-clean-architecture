...
from .search_movements_by_object_filtering_query import SearchMovementsByObjectFilteringQuery
from .search_movements_by_object_ordering_query import SearchMovementsByObjectOrderingQuery


class SearchMovementsByObjectQuery(PaginatedQuery):

    def __init__(self, dto:SearchMovementsByObjectDto):
        super().__init__(
            current_page=dto.current_page,
            page_size=dto.page_size
        )
        self.filtering_criteria = SearchMovementsByObjectFilteringQuery(dto.filtering_criteria) if dto.filtering_criteria is not None else None
        self.ordering_criteria = SearchMovementsByObjectOrderingQuery(dto.ordering_criteria) if dto.ordering_criteria is not None else None
